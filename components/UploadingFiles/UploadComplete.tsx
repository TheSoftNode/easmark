...
}: UploadCompleteProps) =>
{
    ...
    const [recordUsage] = useRecordUsageMutation();
    ...
    //             'https://easemark-upload-check.onrender.com/upload', 
    //             formData, 
    //             {
    ...
    //         const analysisTypeId = uploadType === 'thesis' ? '1' : '2';
    //         const payload = { 
    //             analysis_type: analysisTypeId,
    //             // coupon_code: couponCode || undefined
    //             ...((!userAnalytics?.free_analysis_available && couponCode) ? { coupon_code: couponCode } : {})
    //         };

    //         const result = await performAnalysis(payload).unwrap();
    //         onStartAnalysis(result);

    //     } catch (error: any) {
    //         console.error('Error submitting file for analysis:', error);
    //         setCouponError(error.response?.data?.message || 'Error performing analysis');
    ...
    const handleSubmitForAnalysis = async () =>
    {
        ...
        if (!currentAnalysisType)
        {
            setCouponError('Analysis type not found');
            return;
        }
        ...
        try
        {
            ...
            //     try
            //     {
            //         appliedCoupon = await applyCoupon({ code: couponCode }).unwrap();
            //     } catch (error: any)
            //     {
            //         if (error?.data?.error === "database is locked")
            //         {
            //             // Wait and retry once
            ...
            // 3. Record usage after coupon is applied
            const finalCostAmount = calculateFinalCost();
            if (finalCostAmount > 0)
            {
                try
                {
                    await recordUsage({
                        credit_usage: finalCostAmount
                    }).unwrap();
                } catch (error: any)
                {
                    if (error?.data?.error === "database is locked")
            ...
            // 4. Finally perform the analysis
            try
            {
                const payload = {
                    analysis_type: currentAnalysisType.name,
                    ...((!userAnalytics?.free_analysis_available && couponCode) ?
                        { coupon_code: couponCode } : {})
                };

                const result = await performAnalysis(payload).unwrap();
                console.log('Analysis performed successfully:', result);
                onStartAnalysis(result);
            ...
            // console.log('Sending to performAnalysis:', payload);
                    ...
                    <div className="space-y-4 py-4">
                        {userAnalytics?.free_analysis_available ? (
                            ...
                            <div className="space-y-3">
                                ...
                                {couponDiscount > 0 && (
                                    <div className="flex justify-between items-center text-sm">
                                        <span className="text-violet-600">Discount Applied:</span>
